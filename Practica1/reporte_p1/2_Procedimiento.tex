...
Para determinar la relación carga eléctrica del electrón con la constante de Boltzmann se utilizo la ecuación Shockley para polarización directa:
\begin{equation}
	I_d = I_s \left[ \exp\left(\frac{eV_d}{nk_b T}\right) - 1 \right]
	\label{Shockley1}
\end{equation}

donde $I_s$ es la corriente de saturación inversa, $e$ es la carga del electrón $k_B$ el la constante de Bolztamann, T es la temperatura absoluta del diodo y $n$ es un factor entre 1 y 2, según si el diodo 
opere en el modo difusivo ($n \approx 1$) o recombinativo ($n \approx 2$). Al desarrollar la  ecuación (\ref{Shockley1}) cuando $I_d >> I_s$ obtenemos:
\begin{equation}
...
Una vez obtenida la relación lineal, se procedió a obtener la dupla de datos ($V_d, I_d$) de manera experimental para esto se utilizo un modulo M05, que tienen un circuito eléctrico interno como el que se muestra en la figura (\ref{Circuito M05}) 
\begin{figure}[H]
	\centering
	...
	\label{Circuito M05}
\end{figure}
Primero se conecto la fuente de voltaje de una consola LEB01 a la entrada 1 de la misma, luego para alimentar el modulo M05 este se conecto a el puerto 5 de la consola, proporcionando el voltaje de polarización. Después se conecto un multímetro en paralelo a los puertos del modulo para medir la caída de tensión $V_d$ en el diodo, así como también se conecto otro multímetro en seriecon el circuito, utilizando las salidas 1 y 2 de la consola para medir la corriente directa $I_d$, el arreglo final se muestra se muestra en la figura (\ref{Conexion})
\begin{figure}[H]
	\centering
